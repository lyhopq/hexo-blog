...
-   :split 水平分屏
-   :vsplit 垂直分屏
-   `ctrl+w+dir` dir就是方向，可以是 hjkl 或是 ←↓中的一个，用来切换分屏
-   `ctrl+w+_/|`: 最大化尺寸 （_： 水平，|： 垂直）
-   `ctrl+w++/-`: 增加/减小尺寸
...
```
> 作用：
>    将"(1), ...., (2), ....,(100)"替换成"(2), ...., (3), ...., (101)"，即括号里的序号数字都加一。
>
> 解释：
>    %               全文（"%"是"1，$"范围的缩写）
>    s               替换
...
>    "("             左括号
>    .               字符串连接运算符
>    (submatch(1)+1) 把第一个匹配的结果加1作为一个整体返回
>    .")"    添上右括号
...
> 解释：
> `.,/^$/-1` 选取的范围是自当前行直至下一个空行。也可以在Visual模式下选取要排序的行
...
-   偏移
    -   `/words/2`   将光标停留在目标行向下的第二行
    -   `/words/e-1` "e"使光标在找到目标串后以它的结尾作为移动的起始处
    -   `/words/b+2` "b"目标串开头为起始处
    -   `//e`        重复前一次搜索使用不同的偏移
    -   `?words?e-2` 反向搜索必须以"?"来分割命令的不同部分
...
-   tags
    -   跳转： `CTRL-W ]` 分割当前窗口并跳转到光标下的tag， 
    -   `:tnext` 下一个符合条件的地方
    -   `:tselect tagname` 列出所有符合条件的地方
    -   搜索：`:tag /xxx` ，然后按Tab
    -   预览窗口：`ptag tagname`
    -   关闭窗口`:pclose`
    -   `:pedit defs.h` 在预览窗口中编辑一个文件
    -   `:psearch popen`在预览窗口中显示搜索内容
